...
const ProductDetails = () => {
    ...
    useEffect(()=>{
            if(products.length > 0){
                ...
                relatedProductsCopy = relatedProductsCopy.filter((item) => product.category === item.category)
        ...
        <div className="mt-12">
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                            <img key={i} src={i < 4 ? assets.star_icon : assets.star_dull_icon} alt="ratings" className='md:3.5 w3' />))}
            ...
            <div className='flex flex-col'>
                <div className='flex flex-col items-center mt-20'>
                    <p className='text-3xl font-medium'>Related Producs</p>
                    <div className='w-20 h-0.5 bg-primary rounded-full mt-2'></div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-6 lg:grid-cols-5 mt-6 w-full">
                        {
                            relatedProducts.filter(product => product.inStock).map(product => (<ProductCard key={product._id} product={product}/>))
                        }
                </div>
